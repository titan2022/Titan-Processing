...
if [[ -z "$git_remote"  || -z "$deployment_target" ]]; then
    ...
    echo ""
    echo "Usage: scripts/host-orangepi/no-autostart.sh [git remote] [deployment target]"
...
ssh "titan@$deployment_target" "cd /home/titan/Projects/Titan-Processing && yes titans | ./scripts/orangepi/no-autostart.sh"
