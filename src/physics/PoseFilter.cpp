...
PoseFilter::PoseFilter(ConfigReader &config) :
...
void PoseFilter::updateTag(Apriltag &tag, double tagDist, double dt)
{
    ...
    double xRot = tag.rotation.getX();
    double yRot = tag.rotation.getY();
    double zRot = tag.rotation.getZ();
    ...
    if (!init) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            x.at<double>(i + 3, 0) = tag.rotation[i];
            x.at<double>(i + 6, 0) = 0;
            x.at<double>(i + 9, 0) = 0;
        }
